...
def logistic_regression(x,y):
	...
	while cnt<=100:
		...
		#print(np.log(sigmoid(theta,x)))
		#print(y*np.log(sigmoid(theta,x)+(1-y)*np.log(1-sigmoid(theta,x))))
...
print(x.shape)
theta=theta.T
print(np.dot(sigmoid(theta,x),x))
print(np.dot(x.T,sigmoid(theta,x)))'''
theta=logistic_regression(X,y[0])
#print(theta)
cnt_y=predict(theta,X)
print(np.sum(cnt_y==y[0])/y.shape[1])
